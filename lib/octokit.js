...
module.exports = ({ personalAccessToken }) => {
	...
	/**
	 * Map the yargs command to the equivalent GitHub API command
	 */
	return () => async (command, inputs) => {

		console.dir({ command, inputs })

		const result = await octokit[command](inputs)
		return result.data
	}
}
